...
class Plotter:
    ...
    def plot_fig(self):
        ...
        # Pass the 'reverse' flag to Calculs
        try:
            self.data = Calculs(
                self.info["n"], 
                self.info["ds"], 
...
def main():
    ...
    with tabs[1]:
        ...
        try:
            ...
            # Check attributes before plotting
            ...
            if show_controls and hasattr(data_bezier, 'pts') and data_bezier.pts:
                 # Filter valid points before processing
    ...
    with tabs[4]:
        ...
        st.markdown("This section will display the figures generated using the parameters in the sidebar.")

        col1, col2 = st.columns(2)
        with col1:
            st.info("**Number of sides (n)**")
            st.markdown("Sets the maximum number of sides for the largest polygon. A high `n` value creates a denser, more circular figure.")
        with col2:
            st.info("**Spiral offset (ds)**")
            st.markdown("Determines how the vertices of adjacent polygons connect. `ds=1` creates a smooth spiral. `ds=2` or `ds=3` creates more complex patterns.")

        
        col3, col4 = st.columns(2)
        with col3:
            st.info("**Color Theme**")
            st.markdown("`Random 1` choose a random predefined palette from the matplotlib library. `Random 2` choose random color to make a color radians.")
        with col4:
            st.info("**Elements to Display**")
            st.markdown("Several figures that exploit the same principles: nested polygons with an offset between them. They can be chosen at the same time.")

        col5, col6 = st.columns(2)
        with col5:
            st.info("**Elements to Fill**")
            st.markdown("When `Polygon`, `Inscribed Circle` and `Circumscribed Circle` are chosen in `Elements to Display`, the user can choose to fill these shapes.")
        with col6:
